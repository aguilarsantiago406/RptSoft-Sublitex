...
describe('DisenoService', () => {
  ...
  beforeEach(async () => {
    ...
    (txMock.bloquePedido.upsert as jest.Mock).mockReset();
    ...
    (prismaMock.bloquePedido.findUnique as jest.Mock)
      .mockReset()
      .mockResolvedValue(null);
    (prismaMock.bloquePedido.upsert as jest.Mock)
      .mockReset()
      .mockResolvedValue({});
    (prismaMock.grupo.findMany as jest.Mock)
      .mockReset()
      .mockResolvedValue([
        {
          id: 'g1',
          nombre: 'Grupo 1',
          configuracion: [{ atributoId: 'a1', valorAtributoId: 'v1' }],
        },
      ]);
    (prismaMock.atributo.findMany as jest.Mock)
      .mockReset()
      .mockResolvedValue([
        { id: 'a1', nombre: 'TELA', obligatorio: true, activo: true },
      ]);
  ...
  describe('crear', () => {
    it('crea una versión 1 para un pedido sin versiones previas', async () => {
      txCreateMock.mockResolvedValue({ id: 'd1', pedidoId: 'p1', version: 1 });

      const res = await service.crear({ pedidoId: 'p1' });

      expect(res).toEqual({ id: 'd1', pedidoId: 'p1', version: 1 });
    ...
    it('rechaza crear si el bloque DISENO está cerrado (R-H12)', async () => {
      (prismaMock.bloquePedido.findUnique as jest.Mock).mockResolvedValue({
        estado: 'CERRADO',
      });

      await expect(service.crear({ pedidoId: 'p1' })).rejects.toThrow(
        /bloque de Diseño está cerrado/i,
      );
    });
  });
  ...
  describe('aprobar', () => {
    ...
    it('rechaza aprobar si el pedido no tiene grupos (R-H02)', async () => {
      disenoFindUniqueMock.mockResolvedValue({
        id: 'd1',
        pedidoId: 'p1',
        estado: 'PROPUESTO',
      });
      (prismaMock.usuario.findUnique as jest.Mock).mockResolvedValue({ id: 'u1' });
      (prismaMock.grupo.findMany as jest.Mock).mockResolvedValue([]);

      await expect(service.aprobar('d1', { usuarioId: 'u1' })).rejects.toThrow(
        /atributos obligatorios|grupos configurados/i,
      );
    });

    it('rechaza aprobar si falta configurar un atributo obligatorio (R-H02)', async () => {
      disenoFindUniqueMock.mockResolvedValue({
        id: 'd1',
        pedidoId: 'p1',
        estado: 'PROPUESTO',
      ...
      (prismaMock.usuario.findUnique as jest.Mock).mockResolvedValue({ id: 'u1' });
      (prismaMock.grupo.findMany as jest.Mock).mockResolvedValue([
        { id: 'g1', nombre: 'Grupo 1', configuracion: [] },
      ]);
      (prismaMock.atributo.findMany as jest.Mock).mockResolvedValue([
        { id: 'a1', nombre: 'TELA', obligatorio: true, activo: true },
      ]);

      await expect(service.aprobar('d1', { usuarioId: 'u1' })).rejects.toThrow(
        /no tiene configurado el atributo obligatorio/i,
      );
    });

    it('rechaza aprobar si el bloque DISENO ya estaba cerrado (R-H12)', async () => {
      disenoFindUniqueMock.mockResolvedValue({
        id: 'd1',
        pedidoId: 'p1',
        estado: 'PROPUESTO',
      ...
      await expect(service.aprobar('d1', { usuarioId: 'u1' })).rejects.toThrow(
        /bloque de Diseño ya está cerrado/i,
      );
    });
  });
